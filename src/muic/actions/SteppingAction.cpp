...
namespace muic {
    ...
    void SteppingAction::UserSteppingAction(const G4Step *step) {
        ...
        if (step->IsFirstStepInVolume()) {
            ...
            context_->energy_deposit_collector_.setEtaBeforeNozzle(
                    step->GetTrack()->GetTrackID(),
	
                    prestep_point->GetMomentum().getEta()
		    );
            context_->energy_deposit_collector_.setPhiBeforeNozzle(
                    step->GetTrack()->GetTrackID(),
	    ...
	    context_->energy_deposit_collector_.setThetaBeforeNozzle(
			    step->GetTrack() ->GetTrackID(),
	    ...
	    context_-> energy_deposit_collector_.setPositionThetaBeforeNozzle(
            ...
            context_->energy_deposit_collector_.setEtaAfterNozzle(
                    step->GetTrack()->GetTrackID(),
		   
                    poststep_point->GetMomentum().getEta()
            );
            context_->energy_deposit_collector_.setPhiAfterNozzle(
                    step->GetTrack()->GetTrackID(),
    ...
    context_->energy_deposit_collector_.setPositionEtaAfterNozzle(
	    ...
	    context_ ->energy_deposit_collector_.setThetaAfterNozzle(
			    step->GetTrack() -> GetTrackID(),
